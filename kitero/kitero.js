...
function hideBasedOnSelected(dropdownList){
    ...
    const szulo = dropdownList.parentElement;
    const lista = szulo.querySelectorAll(".card");
    
    for (const elem of lista) {
        elem.classList.add("hide");
    };
    makeVisibleBasedOnId(szulo, ertek);
}
...
function hideBasedOnCheckbox(checkbox){
    const bepipalva = checkbox.checked;
    const szulo = checkbox.parentElement;
     
    if (bepipalva) {
        makeInvisibleBasedOnId(szulo, "no");
        makeVisibleBasedOnId(szulo, "ferfi");
    }
    else {
        makeInvisibleBasedOnId(szulo, "ferfi");
        makeVisibleBasedOnId(szulo, "no");
    }
}
